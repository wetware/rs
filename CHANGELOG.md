...
- **Post-install daemon probe + signpost.** `perform_install` polls `http://127.0.0.1:2080/status` with backoff (~10 s budget total) after `perform_update` returns. When the daemon answers with HTTP 200, the post-install summary's "Next steps" line becomes `curl http://localhost:2080/status` (was `ww shell`). launchd / systemd route daemon stdout to `~/.ww/logs/ww.log`, so any boot-time log line never reaches the install transcript — the probe path puts the URL where the user can actually see it.
