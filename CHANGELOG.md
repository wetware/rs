...
### Added
- **Guest-extras schemas in `Export.schema` (#386, Item 1b — closes #386).** The graft loop's extras path now populates `Export.schema` for init.d-scoped capability grants, completing the work started by Item 1a. Schemas are caller-provided: when the host receives an extra cap over `Executor.spawn` or `VatListener.listen`, it canonicalizes the wire-side `Schema.Node` reader (via the new `crate::rpc::canonicalize_schema_node` helper) and threads the bytes through `HostGraftBuilder::with_extras`. The previously-private `bytes_to_aligned_words` helper is now `pub(super)` and reused across the core-cap path, the extras path, and the vat-listener forward path. Empty-bytes case is handled explicitly: graft emits a `tracing::warn!` and falls back to an empty `Schema.Node`. The wire format already carried these bytes (`stem.capnp:Export.schema`) — this PR plugs the four drop points where they were previously discarded.
