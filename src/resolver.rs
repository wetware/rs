...
impl ServiceResolver {
    ...
    async fn resolve_ipfs_service(&mut self, service_path: &str) -> Result<Vec<u8>> {
        ...
        let path_parts: Vec<&str> = service_path.trim_start_matches("/ipfs/").split('/').collect();
    ...
    async fn resolve_ipfs_path(&mut self, root_hash: &str, service_path: &str) -> Result<Vec<u8>> {
        ...
        if let Some(file_hash) = dir_structure.get(service_path) {
            ...
            self.find_service_in_ww_structure(root_hash, service_path).await
        }
    }
    
    /// Find a service within the /ww/<version>/public/ structure
    async fn find_service_in_ww_structure(&mut self, root_hash: &str, service_path: &str) -> Result<Vec<u8>> {
        ...
        if let Some(ww_hash) = root_structure.get("ww") {
            ...
            let mut versions: Vec<&String> = ww_structure.keys()
            ...
            if let Some(latest_version) = versions.first() {
                if let Some(version_hash) = ww_structure.get(*latest_version) {
                    ...
                    if let Some(public_hash) = version_structure.get("public") {
                        let public_structure = self.get_ipfs_directory_structure(public_hash).await?;
        ...
        Err(anyhow!("Service not found in IPFS structure: {}", service_path))
    }
    
    /// Get directory structure for an IPFS hash
    async fn get_ipfs_directory_structure(&mut self, hash: &str) -> Result<HashMap<String, String>> {
        // Check cache first
        ...
        // Cache the result
        self.ipfs_dir_cache.insert(hash.to_string(), structure.clone());
    ...
    async fn fetch_ipfs_directory_listing(&self, hash: &str) -> Result<HashMap<String, String>> {
        ...
        if !response.status().is_success() {
            return Err(anyhow!("IPFS directory listing failed: {}", response.status()));
        }
        ...
        if let Some(objects) = json["Objects"].as_array() {
            for obj in objects {
                if let Some(links) = obj["Links"].as_array() {
                    for link in links {
                        if let (Some(name), Some(hash)) = (link["Name"].as_str(), link["Hash"].as_str()) {
...
mod tests {
    ...
    fn test_version_detection() {
        ...
        let resolver = ServiceResolver::new(temp_dir.path().to_path_buf(), "http://localhost:5001".to_string());
    ...
    async fn test_service_resolution() {
        let temp_dir = TempDir::new().unwrap();
        let mut resolver = ServiceResolver::new(temp_dir.path().to_path_buf(), "http://localhost:5001".to_string());
        
        // Create test service
        fs::create_dir_all(temp_dir.path().join("ww/0.1.0/public")).unwrap();
        fs::write(temp_dir.path().join("ww/0.1.0/public/echo.wasm"), b"wasm_content").unwrap();
    ...
    async fn test_nested_service_resolution() {
        ...
        fs::write(temp_dir.path().join("ww/0.1.0/public/math/add.wasm"), b"add_wasm_content").unwrap();
